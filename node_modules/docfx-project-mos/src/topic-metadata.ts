    ...
    switch (metadata.type) {
        ...
        case 'Reference.Managed': {
            if (metadata.memberType)
            {
                ...
                return commentIdToTopicType(metadata.commentId);
        }
...
interface ManagedReferenceRoot {
    ...
    references: ManagedReferenceMetadata[];
}
...
async function parseManagedReferenceYaml(fileName: string): Promise<TopicMetadata[]> {
    ...
    const mrefYaml = await readYaml<ManagedReferenceRoot>(fileName,
        'XRefMap' // Expected YAML MIME type.
    );
    if (!mrefYaml || !mrefYaml.references)
        return topicMetadata;

    for (const managedReference of mrefYaml.references) {
        ...
        topicMetadata.push({
            ...
            name: managedReference.name,
            title: managedReference.nameWithType || managedReference.name,
            // sourceFile: "../_siteGitLab/" + managedReference.href
            sourceFile: managedReference.href,
            commentId: managedReference.commentId            
        });
    }
...
function commentIdToTopicType(commentId: string): TopicType
{
    // _INFO_: mos 20221017: this is a fix for issue with conceptual files (i.e., markdown files) having a YAML header, that specifies the uid (for x-referencs from source code via '<see href="xref:<uid>"/> ),
    //                       but for which no 'commentId'-field will be generated in the xrefmap.yml-file (and therefore accessing 'commentId[0]' would throw an exception)
    if (!commentId) return TopicType.Conceptual;

    const typeSymbol = commentId[0];
    switch (typeSymbol) {
        case 'N': {
            return TopicType.Namespace;
        }
